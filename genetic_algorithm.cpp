...
Agent::Agent(const float& afitness, const PeepoNetwork& a_network, const float& atopp, const float& acpdp) :
	fitness(afitness),
	network(a_network),
	mut_top(atopp),
	mut_cpd(acpdp)
...
std::vector<json> get_topologies(PeepoNetwork& peepo, const unsigned& npop)
{
	...
	std::uniform_real_distribution<float> di(0.0, 2.0*0.999999999999);//uniform distribution in interval [0,2)
	int count = 0;
	while (true) {
...
GeneticAlgorithm::GeneticAlgorithm(
	...
	const float& convergence_sensitivity_percent,
	const unsigned&  Npop,
	const float& p_mut_top,
	const float& p_mut_cpd,
	...
	convergence_ratio(float(Npop * POPULATION_CONVERGENCE_RATIO)),
...
std::vector<float> GeneticAlgorithm::generate_rand_vec(const unsigned& dim, const float& coef)
{
	std::default_random_engine dre(seed);//engine
	std::uniform_real_distribution<float> di(0.0, coef);//distribution
	std::vector<float> my_rand(dim);
...
void GeneticAlgorithm::initialize(const std::string& origin)
{
	...
	for (unsigned t = 0; t < topologies.size();t++) {
		...
		for (auto node : peepo.get_nodes()) {
			...
			t_cpd_root  root_cpd;
			t_cpd_leaf leaf_cpd;
			std::vector<float> my_omega;
			if (incoming_nodes.size() == 0) {
				for (unsigned n = 0; n < cardinality_map[node]; n++) {
					root_cpd.push_back(1.0 / float(cardinality_map[node]));
				...
				float max_omega = 2.0*PI*prod;
				std::vector<float> my_omega = generate_rand_vec(cardinality_map[node], max_omega);
...
bool GeneticAlgorithm::check_convergence(void)
{
	...
	else
	{
		...
		double std = 0.0;
		{
			...
			for (auto fit : convergence_history) { [&mean, len](const float& fit) {mean += fit / len; }; };
			for (auto fit : convergence_history) { [&std, mean](const float& fit) {std += (fit - mean)*(fit - mean); }; }
			std = std::sqrt(std / len);
		}//scope bracket
		double acceptable_std = mean * convergence_sensitivity / 2.0;
		if (std < acceptable_std) { return true; }
	}
	float prev_fitness = last_generation.begin()->first;
	float actual_fitness = population_fitness_score;
	...
	last_generation.clear();
	last_generation.insert(std::pair<float, std::vector<Agent> >(actual_fitness, selected_parents));
...
std::vector<Agent> GeneticAlgorithm::evolve(std::vector<Agent> in_population, float& in_fitness, bool& converging)
{
	...
	for (unsigned offspr = 0; offspr < selected_offsprings.size(); offspr++) {
		...
		float m_top = offspring.mut_top;
		float m_cpd = offspring.mut_cpd;
		...
		for (auto comb : combination) {
			...
			for (auto node : a_peepo.get_nodes()) {
				...
				if (incoming_nodes.size() == 0) {
					...
					float max_omega = 2.0*PI*prod_card;
				...
				float mut_top = float(std::rand() % 100) / 100.0;
				float mut_cpd = float(std::rand() % 100) / 100.0;
				if (comb[0] == comb[1]) {
				mut_top = 0.0;
				mut_cpd = 0.0;
				}
...
PeepoNetwork GeneticAlgorithm::mutate_cpds(PeepoNetwork network)
{
	...
	float epsilon = 0.05 + float(std::rand() % 1000 / 1000.0*0.7);
	for (auto leaf : leafs) {
		...
		std::vector<float>  my_omega = omega_map[leaf].get<std::vector<float>>();
		for (unsigned i = 0; i < my_omega.size(); i++) {
			my_omega[i] += (0.5 - float(std::rand() % 1000) / 1000.0)*epsilon;
		}
...
t_cpd_leaf GeneticAlgorithm::ga_child_cpd(const std::vector<unsigned>& card_parents, const std::vector<float>& omega)
{
	t_cpd_leaf pdf;
	float phase_shift = omega[0];
	unsigned n_comb = 1;
