...
-------------------
*/
typedef variant<std::vector<std::vector<float> >, std::vector<float> > t_cpd;//generic type for any cpds
typedef std::vector<std::vector<float> > t_cpd_leaf;//leaf node cpds type
typedef std::vector<float> t_cpd_root;//root node cpds type
...
public://functions
	...
	std::map<std::string, std::vector<float> > get_inference(std::map<std::string, unsigned>&);
