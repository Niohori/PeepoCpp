...
{
	Agent(void);
	Agent(const float&, const PeepoNetwork&, const float& = 0.0, const float& = 0.0);
...
// ----------------------------------------------------------------------------------------
	...
	float POPULATION_CONVERGENCE_RATIO = 1. / 5.;
	int NUMBER_OF_MATING_PARENTS = 2;
	float PI = 3.141596;

	float population_fitness_score;

private://functions
	std::vector<float> generate_rand_vec(const unsigned&, const float&);
...
public://members
	...
	std::map<float, std::vector<Agent> > last_generation;
	unsigned convergence_period;
	std::vector<float> convergence_history;
	float convergence_sensitivity;
...
public://functions
	...
	std::vector<Agent> evolve(std::vector<Agent>, float&, bool&);
	std::vector<Agent> cross_over(void);
	PeepoNetwork prune_or_grow(PeepoNetwork);
	PeepoNetwork mutate_cpds(PeepoNetwork);
	t_cpd_leaf ga_child_cpd(const std::vector<unsigned>&, const std::vector<float>&);
